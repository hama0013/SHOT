...
def keypress(channel):
    ...
    if status == 0:       # aus Freizeit kommend, Startzeit speichern
        ...
        f= open("/home/pi/shot/shot.csv","a+")    #CSV-Datei oeffnen
...
def main():
    ...
    shot_path = "/home/pi/shot/"
    shot_file = "shot.csv"

    folder_exists = os.path.isdir(shot_path)             #Pruefe Ordner Verfuegbarkeit
    file_exists = os.path.isfile(shot_path+shot_file)     #Pruefe Datei Verfuegbarkeit

    if folder_exists == False:  # Ordner fehlt, dann
        os.mkdir(shot_path)      # Lege Ordner an
        os.chmod(shot_path , stat.S_IRWXU|stat.S_IRWXG|stat.S_IRWXO)   #umfassende Schreibrechte auf den Ordner

    
    if file_exists: 
        print("CSV-File exists\r\n")
    else:   #Datei fehlt, dann
        f= open("/home/pi/shot/shot.csv","a+") #Lege Datei an
        ...
        os.chmod("/home/pi/shot/shot.csv" , stat.S_IRWXU|stat.S_IRWXG|stat.S_IRWXO)   #umfassende Schreibrechte auf die Datei
